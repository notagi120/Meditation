...
// nullであれば、その後の処理は実行されません。これは、countDownTimerが初期化されていない可能性があるため、エラーを防ぐためのチェックです。
    ...
    private void manageBreathingCycle(long totalTimeMillis, long millisUntilFinished, long breatheInSec, long breatheOutSec, long cycleMillis) {
        ...
        //以下、具体例
//        全体の呼吸サイクルが5回繰り返すとします。吸う時間が3秒、吐く時間が7秒なので、1サイクルにかかる時間は10秒です。
//        全体の時間は、5回繰り返すので、50秒（totalTimeMillis = 50000ミリ秒）になります。
//        この例で、残り時間がmillisUntilFinished = 32000ミリ秒（32秒）だとしましょう。
//        1サイクルにかかる時間はcycleMillis = 10000ミリ秒（10秒）です。
//        この場合、以下の計算になります：
//        合計時間から残り時間を引く
//        50000−32000=18000ミリ秒。これは、これまでに経過した時間です。
//        経過した時間を1サイクルの時間で割った余りを取る：
//        18000%10000=8000ミリ秒。これは、現在のサイクル内での経過時間（timeInCurrentCycle）です。
//        この計算の結果、現在のサイクルは途中で、吐くフェーズの途中にいることがわかります。
// 吸う・吐くサイクルの管理ロジック
